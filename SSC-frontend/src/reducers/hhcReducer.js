...
export const fetchTeam = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get('https://ssc-final-backend.onrender.com/api/postHolders/hhc');
...
export const fetchEvents = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get('https://ssc-final-backend.onrender.com/api/events/hhc');
...
const initialState = {
  ...
  const hhcReducer = (state = initialState, action) => {
    switch (action.type) {
