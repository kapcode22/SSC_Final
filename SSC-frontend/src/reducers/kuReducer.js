...
export const fetchTeam = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get('https://ssc-final-backend.onrender.com/api/postHolders/ku');
...
export const fetchEvents = () => async (dispatch) => {
  ...
  try {
    const res = await axios.get('https://ssc-final-backend.onrender.com/api/events/ku');
...
const initialState = {
  ...
  const kuReducer = (state = initialState, action) => {
    switch (action.type) {
