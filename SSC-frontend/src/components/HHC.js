...
const Hhc = () => {
  ...
  const fetchCarouselImages = async () => {
    try {
      const response = await axios.get('https://ssc-final-backend.onrender.com/api/sliderImages/hhc');
            ...
            {carouselImages && carouselImages.length > 0 ? (
              carouselImages.map((carouselimage, index) => (
                <div key={index} className={`carousel-item ${index === 0 ? 'active' : ''}`}>
                  <img className={hstyles.carousel_img} src={`https://ssc-final-backend.onrender.com/${carouselimage.imageUrl.replace(/\\/g, '/')}`} alt="" />
                </div>
              ))
            ) : (
      ...
      <div className={`${hstyles.events} ${hstyles.container}`}>
        ...
        {events.map((event, index) => (
          <div className={hstyles.event} key={index}>
            ...
            <img className={hstyles.img_ani} src={`https://ssc-final-backend.onrender.com/${event.image.replace(/\\/g, '/')}`} alt="" />
